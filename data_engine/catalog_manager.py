...
class CatalogManager:
    ...
    def load_catalog_delta(self, run_id: str):
        """カタログデルタのみをロード（Bin データは読み込まない）"""
        return self.delta.load_catalog_delta(run_id)

    def iter_bin_deltas(self, run_id: str):
        """Bin デルタを1キーずつ逐次ロードする"""
        return self.delta.iter_bin_deltas(run_id)
    ...
    # ──────────────────────────────────────────────
    _PROCESSED_STATUSES = frozenset({"success", "retracted", "english_empty", "attachment_empty", "unsupported_taxonomy"})

    def _rebuild_lookup_caches(self):
        """カタログDFから高速ルックアップ用のキャッシュを再構築する（起動時用）"""
        ...
        self._processed_set = {
            ...
            if status in self._PROCESSED_STATUSES
        }

    def _update_lookup_caches(self, df_new: "pd.DataFrame"):
        """新規レコードのみでキャッシュを差分更新する（update_catalog用）

        起動時に _rebuild_lookup_caches() で全件構築済みのキャッシュに対し、
        今回のデルタ分だけを追加・上書きする。全件再構築を回避しメモリを節約する。
        """
        for doc_id, status in zip(df_new["doc_id"], df_new["processed_status"]):
            self._status_cache[doc_id] = status
            if status in self._PROCESSED_STATUSES:
                self._processed_set.add(doc_id)
            else:
                self._processed_set.discard(doc_id)
    ...
    def get_raw_zip_path(self, doc_id: str):
        """カタログに記録された raw_zip_path を取得する。"""
        return self._get_catalog_val(doc_id, "raw_zip_path")

    def get_pdf_path(self, doc_id: str):
        """カタログに記録された pdf_path を取得する。"""
        return self._get_catalog_val(doc_id, "pdf_path")

    def get_english_path(self, doc_id: str):
        """カタログに記録された english_path を取得する。"""
        return self._get_catalog_val(doc_id, "english_path")

    def get_attach_path(self, doc_id: str):
        """カタログに記録された attach_path を取得する。"""
        return self._get_catalog_val(doc_id, "attach_path")

    def _get_catalog_val(self, doc_id: str, col: str):
        if self.catalog_df.empty:
            return None
    ...
    def update_catalog(self, new_records: List[Dict]):
        ...
        # 【メモリ最適化】df_new 内の重複を先に排除し、既存カタログから
        # 更新対象の doc_id を除去してから追記する。
        # concat + drop_duplicates(keep="last") と数学的に同一の結果だが、
        # 全件コピー + 内部コピーを回避しピークメモリを約半減する。
        df_new = df_new.drop_duplicates(subset=["doc_id"], keep="last")

        if self.catalog_df.empty:
            self.catalog_df = df_new
        else:
            mask = self.catalog_df["doc_id"].isin(df_new["doc_id"])
            self.catalog_df = pd.concat(
                [self.catalog_df[~mask], df_new], ignore_index=True
            )

        self._update_lookup_caches(df_new)  # 差分キャッシュ更新

...
class EdinetPurgedError(Exception):
    """EDINET API が書類をパージ（時期超過等）している際に送出される例外"""
    pass
    ...
    def download_doc(self, doc_id: str, save_path: Path, doc_type: int = 1) -> bool:
        ...
        try:
            ...
            if res.status == "success":
                ...
                # 404 (JSON) の場合は専用のエラーを投げて上位に通知する
                if res.message and "JSON error payload" in res.message:
                    raise EdinetPurgedError(f"EDINET API reported document as purged: {res.message}")

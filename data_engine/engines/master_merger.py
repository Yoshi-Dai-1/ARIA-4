...
from data_engine.core.models import MERGE_BATCH_SIZE_DEFAULT, MERGE_BATCH_SIZES
...
class MasterMerger:
    ...
    def merge_and_upload(
        ...
        try:
            t0 = time.monotonic()
            logger.info(f"  📥 HF Download 開始: {repo_path} ({len(new_data)} 行マージ予定)")
            m_path = hf_hub_download(repo_id=self.hf_repo, filename=repo_path, repo_type="dataset", token=self.hf_token)
            t1 = time.monotonic()
            file_size_mb = Path(m_path).stat().st_size / (1024 * 1024)
            logger.info(f"  📥 HF Download 完了: {file_size_mb:.1f} MB ({t1 - t0:.1f}s)")
            logger.info(f"  🔀 Merge 開始: bin={bin_id} ({master_type})")
            self._merge_with_duckdb(m_path, new_data, master_type, bin_id, local_file)
            t2 = time.monotonic()
            output_size_mb = local_file.stat().st_size / (1024 * 1024)
            logger.info(f"  🔀 Merge 完了: {output_size_mb:.1f} MB ({t2 - t1:.1f}s) | 合計: {t2 - t0:.1f}s")
    ...
    def _merge_with_duckdb(
        ...
        2. マスタを MERGE_BATCH_SIZES[master_type] 行ずつ読み込み、
           新データと重複するキーの行を除外
        3. 除外後の行を出力ファイルに書き出す
        ...
        try:
            merge_batch = MERGE_BATCH_SIZES.get(master_type, MERGE_BATCH_SIZE_DEFAULT)
            for batch in reader.iter_batches(batch_size=merge_batch):
                ...
                if len(filtered) > 0:
                    ...
                    del aligned
                del table, key_lists, keep, filtered

...
class MasterMerger:
    ...
    def merge_and_upload(
        ...
        try:
            ...
            # 【重要】物理的にファイルが存在しない場合 (404) のみ新規作成を許容する。
            # LocalEntryNotFoundError や 503 等の通信エラー時は、既存データを救うため例外をそのまま投げる（中断）。
            is_true_404 = not isinstance(e, LocalEntryNotFoundError) and (
            ...
            logger.error(f"Masterロード中に予期しないエラーが発生しました: {e}")
            raise e

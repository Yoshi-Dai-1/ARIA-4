...
from data_engine.core.network_utils import is_404, patch_all_networking
from data_engine.core.utils import get_safe_str, get_tar_repo_path, normalize_code, parse_datetime
from data_engine.storage.tar_bundle_manager import extract_directory_from_tar, extract_file_from_tar
...
class WorkerEngine:
    ...
    def _apply_status(self, record, new_status):
        ...
        数値が小さいほど優先度が高く、より確定的な判定を意味する。
        """
        hierarchy = {
            "retracted": 1,               # 取下げ（最高優先度）
            "unsupported_taxonomy": 2,     # 構造的に対応不能（永久的）
            "failure": 3,                  # 解析失敗（一時的、リトライ可能）
            "english_empty": 4,            # 英文空書類
            "attachment_empty": 5,         # 添付空書類
            "parsed": 6,                   # 解析済み（未コミット）
            "success": 7,                  # 完全成功（最低優先度）
        }
    ...
    def _is_legacy_taxonomy(self, zip_path: Path) -> bool:
        """旧EDINETタクソノミ (jpfr, 2013年以前) を ZIP 内のファイル名パターンで検知する。

        物理的事実: 旧タクソノミは XBRL/jpfr-* というファイル名を持ち、
        新タクソノミ (jpcrp/jppfs) の PublicDoc/ 構造とは根本的に異なる。
        このプレフィックスは旧タクソノミの唯一無二の識別子であり、
        新タクソノミの書類で使われることは仕様上ありえない。
        """
        try:
            with zipfile.ZipFile(zip_path) as zf:
                return any(name.startswith("XBRL/jpfr-") for name in zf.namelist())
        except Exception:
            return False
    ...
    def run(self):
        ...
        for row in all_meta:
            ...
            if not zip_ok:
                ...
                if catalog_zip:
                    try:
                        ...
                        if is_404(e):
                            # 【TAR フォールバック】個別ファイルが 404 の場合、TAR バンドルから抽出を試みる
                            tar_repo = get_tar_repo_path(row.get("submitDateTime", ""))
                            if tar_repo:
                                try:
                                    ...
                                    if extract_file_from_tar(Path(tar_local), f"zip/{doc_id}.zip", raw_zip):
                                        if zipfile.is_zipfile(raw_zip):
                                            zip_ok = True
                                            xbrl_flag = True
                                            row["xbrlFlag"] = "1"
                                            verdict = ProcessVerdict.PARSE
                                            logger.info(f"ZIP Recovered from TAR: {tar_repo} -> {doc_id}")
                                        else:
                                            raw_zip.unlink(missing_ok=True)
                                            anomaly_status = "failure"
                                except Exception as tar_e:
                                    if is_404(tar_e):
                                        logger.error(f"HF ZIP missing (individual & TAR) for {doc_id}. Recovery impossible.")
                                    else:
                                        logger.critical(f"Aborting Worker due to network error during TAR ZIP recovery: {tar_e}")
                                        raise
                            if not zip_ok:
                                anomaly_status = "failure"
                        else:
                            # 503 等の通信障害時は、カタログを failure に書き換えず、プロセスを落としてリトライさせる
                            logger.critical(f"Aborting Worker due to network error during ZIP recovery: {e}")
                            raise
                        ...
                        if is_404(e):
                            ...
                            if tar_repo:
                                try:
                                    ...
                                    if extract_file_from_tar(Path(tar_local), f"pdf/{doc_id}.pdf", raw_pdf):
                                        if self._is_pdf(raw_pdf):
                                            pdf_ok = True
                                            logger.info(f"PDF Recovered from TAR: {tar_repo} -> {doc_id}")
                                        else:
                                            raw_pdf.unlink(missing_ok=True)
                                except Exception as tar_e:
                                    if not is_404(tar_e):
                                        logger.critical(f"Aborting Worker due to network error during TAR PDF recovery: {tar_e}")
                                        raise
                            if not pdf_ok:
                                logger.error(f"HF PDF missing (individual & TAR) for {doc_id}.")
                                raw_pdf.unlink(missing_ok=True)
                        else:
                            logger.critical(f"Aborting Worker due to network error during PDF recovery: {e}")
                            raise
                            ...
                            if is_404(e):
                                ...
                                if tar_repo:
                                    try:
                                        ...
                                        cnt = extract_directory_from_tar(
                                            Path(tar_local), f"english/{doc_id}/", eng_dir
                                        )
                                        if cnt > 0:
                                            ...
                                            if not rel_english_path.endswith("/"):
                                                rel_english_path += "/"
                                            eng_ok = True
                                            logger.info(f"English Recovered from TAR: {tar_repo} -> {doc_id} ({cnt} files)")
                                    except Exception as tar_e:
                                        if not is_404(tar_e):
                                            logger.critical(f"Aborting Worker due to network error during TAR English recovery: {tar_e}")
                                            raise
                                if not eng_ok:
                                    logger.error(f"HF English docs missing (individual & TAR) for {doc_id}.")
                            else:
                                logger.critical(f"Aborting Worker due to network error during English recovery: {e}")
                                raise
                            ...
                            if is_404(e):
                                ...
                                if tar_repo:
                                    try:
                                        ...
                                        cnt = extract_directory_from_tar(
                                            Path(tar_local), f"attach/{doc_id}/", attach_dir
                                        )
                                        if cnt > 0:
                                            ...
                                            if not rel_attach_path.endswith("/"):
                                                rel_attach_path += "/"
                                            att_ok = True
                                            logger.info(f"Attachment Recovered from TAR: {tar_repo} -> {doc_id} ({cnt} files)")
                                    except Exception as tar_e:
                                        if not is_404(tar_e):
                                            logger.critical(f"Aborting Worker due to network error during TAR Attachment recovery: {tar_e}")
                                            raise
                                if not att_ok:
                                    logger.error(f"HF Attachment docs missing (individual & TAR) for {doc_id}.")
                            else:
                                logger.critical(f"Aborting Worker due to network error during Attachment recovery: {e}")
                                raise
            ...
            if verdict == ProcessVerdict.PARSE and zip_ok:
                # 【旧タクソノミ早期検知】jpfr (2013年以前) は構造的に対応不能
                if self._is_legacy_taxonomy(raw_zip):
                    logger.warning(
                        f"【旧タクソノミ検知】{doc_id}: XBRL/jpfr-* パターンを検出。"
                        f"旧EDINETタクソノミは未対応のため、解析をスキップします。"
                    )
                    self._apply_status(record, "unsupported_taxonomy")

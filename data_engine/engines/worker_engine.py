...
from data_engine.core.utils import get_safe_int, get_safe_str, normalize_code, parse_datetime
    ...
    def run(self):
        ...
        for row in all_meta:
            ...
            if submit_date:
                ...
                logger.warning(f"Unparseable submitDateTime '{row.get('submitDateTime')}' for {doc_id}. Saving to unknown.")
            ...
            if xbrl_flag:
                ...
                if zip_ok and not zipfile.is_zipfile(raw_zip):
                    logger.warning(f"ZIP integrity check failed for {doc_id}: downloaded file is not a valid ZIP archive. Falling back to HF.")
                        ...
                        try:
                            if catalog_eng.endswith("/"):
                                ...
                                if extracted_count > 0:
                                    ...
                                    logger.info(f"English Directory Verified/Recovered from HF: {catalog_eng} ({extracted_count} files)")
                                else:
                        ...
                        try:
                            if catalog_att.endswith("/"):
                                ...
                                if extracted_count > 0:
                                    ...
                                    att_ok = True
                                    logger.info(f"Attachment Directory Verified/Recovered from HF: {catalog_att} ({extracted_count} PDFs)")
                                else:

...
class TaxonomyMappingEngine:
    ...
    def generate_mapping_dataframe(self) -> pd.DataFrame:
        ...
        # --- ヘッダー行の完全排除 ---
        # Excelの仕様上、複数の表が同一シートに「縦巻き」されている場合に伴う途中ヘッダー行の紛れ込みを数学的に排除する
        final_df = final_df[
            (final_df["element_name"] != "要素名") & 
            (final_df["namespace"] != "名前空間プレフィックス")
        ].copy()

        # --- NULL 基底アーキテクチャの統治 ---
        # 公式Excelがカテゴリを持たないタイトル行等に付与する "-" というダミー記号を、厳格な NULL に置換する
        if "category_code" in final_df.columns:
            final_df.loc[final_df["category_code"] == "-", "category_code"] = None
                ...
                missing_entries.append(
                    {
                        ...
                        "taxonomy_year": "0000_bridge",  # 工学的主権: 年度ソートで最下位とし実録データを優先
                    }
                )
        if missing_entries:
        ...
        # --- 要素名を一意キーとし重複排除（Cartesian JOIN 防御）---
        # 同じ element_name が複数シート（別業種や別namespace）にまたがって存在する場合、
        # Zero-ETL (DuckDB) での直積爆発を防ぐため、単一の代表メタデータとして重複を完全に排除する。

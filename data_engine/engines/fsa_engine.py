...
class FsaEngine:
    ...
    def sync_edinet_code_lists(self) -> Tuple[Dict[str, EdinetCodeRecord], Dict[str, str]]:
        ...
        try:
            ...
            # 【工学的主権】404 (欠落) 以外の一時的な通信障害 (503/504 等) 時は raise し、
            # 空のリストを返して下流工程でマスタを全削除してしまうリスクを物理的に遮断する。
            if is_404(e):
                logger.warning(f"EDINETコードリストが物理的に不在（404）です: {e}")
                return results, agg_map
            
            logger.critical(f"通信障害のため EDINETコードリストの同期を中断します（Fail-Fast）: {e}")
            raise e

...
class HfStorage:
    ...
    def load_parquet(self, key: str, clean_fn=None, force_download: bool = False, revision: str = None) -> pd.DataFrame:
        ...
        # 【GHA Artifact 優先ロード】
        # Worker ジョブでは Discovery が取得済みのデータが GHA Artifact 経由で
        # ローカルに配置されている。HF への冗長アクセスを完全に排除する。
        local_file = self.data_path / filename
        if local_file.exists() and not force_download and not revision:
            try:
                df = pd.read_parquet(local_file)
                if clean_fn:
                    df = clean_fn(key, df)
                logger.info(f"ローカルキャッシュからロード: {filename} ({len(df)} rows)")
                return df
            except Exception as e:
                logger.warning(f"ローカルキャッシュの読み込みに失敗しました。HFからダウンロードします: {e}")
                # 失敗した場合はそのまま下のHFダウンロード処理（フォールバック）へ進む
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    # 【多層防御・第2層】429/レート制限を is_404 より先に検出する。
                    # hf_hub_download は HEAD リクエストの 429 失敗を EntryNotFoundError として
                    # ラップする場合があり、is_404() で先に判定すると「ファイル不在」と誤認される。
                    # 文字列レベルでの 429 検出を最優先とし、リトライに回す。
                    status_code = getattr(getattr(e, "response", None), "status_code", None)
                    err_str = str(e)
                    if status_code == 429 or "429" in err_str:
                        wait_time = (attempt + 1) * 30 + random.uniform(5, 15)
                        logger.warning(
                            f"HF Load Rate Limit (429). Waiting {wait_time:.1f}s... "
                            f"({attempt + 1}/{max_retries})"
                        )
                        time.sleep(wait_time)
                        continue

                    # 404 (ファイル不在) の場合はリトライ不要 — 上位の最終判定層へ委譲
                    if is_404(e):
                        raise e
                    
                    if attempt < max_retries - 1:
                        wait_time = (attempt + 1) * 10
                        logger.warning(f"HF Load Error ({e}). Retrying in {wait_time}s...")
                        time.sleep(wait_time)
                        continue
                    raise e
        except Exception as e:
            # 【最終判定層】is_404 ヘルパーを使用して、新規作成（404）か異常（その他）かを決定
            if not is_404(e):
                # 通信障害 (503/504 等) の場合は、歴史データを守るため「Fail-Fast」で停止
                logger.critical(f"Aborting load due to network error (Not a 404): {e}")
                raise e
        ...
        return pd.DataFrame()  # 型安全のための最終フォールバック
    ...
    def push_commit(self, message: str = "Batch update from ARIA") -> bool:
        ...
        ops_list = []
        tail_ops = [] # カタログなどの最重要ファイルを最後にコミットするための退避リスト

        catalog_path = self.paths.get("catalog", "catalog/documents_index.parquet")
        master_path = self.paths.get("master", "master/stocks_master.parquet")
        ...
        for repo_path, data in self._commit_operations.items():
            ...
            if repo_path in (catalog_path, master_path):
                tail_ops.append(op)
            else:
                ops_list.append(op)

        # 最重要操作を配列の末尾（最終バッチ）に結合する
        ops_list.extend(tail_ops)
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if isinstance(e, Exception):
                        ...
                        # 【Fail-Fast】400 Bad Request は構造的なエラー（ファイル数上限超過等）。
                        # リトライしても解決しないため、即座にプロセスを停止する。
                        if status_code == 400:
                            resp_text = getattr(resp, "text", "") or ""
                            logger.critical(
                                f"⛔ Fail-Fast: HF が 400 Bad Request を返しました。"
                                f"ファイル数上限 (1,000,000) に達した可能性があります。"
                                f"\n  Batch {i + 1}, Response: {resp_text[:300]}"
                            )
                            return False

                        if status_code in [429, 500]:
                            headers = getattr(resp, "headers", {})
                            wait_time = int(getattr(headers, "get", lambda x, y: y)("Retry-After", "60"))
    ...
    def _upload_with_retry(self, local_path_str: str, repo_path: str, max_retries: int = 5) -> bool:
        ...
        api = self.api  # ローカル変数にコピーして型を確定させる
        for attempt in range(max_retries):
            try:
                api.upload_file(

...
from huggingface_hub.utils import EntryNotFoundError, HfHubHTTPError, LocalEntryNotFoundError, RepositoryNotFoundError
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    # 404 やリポジトリ不在はリトライしても無駄なので即座に re-raise
                    raise e
            ...
            # 【重要】EntryNotFoundError は LocalEntryNotFoundError (ネットワークエラー) の親クラスであるため、
            # 明確に HTTP 404 ステータスコードが返された場合のみを「ファイル不在」と見なす。
            # 503 や 504 などの一時的な通信エラーを 404 と誤認し、既存データを破壊するのを防ぐ。
            is_404 = (
                isinstance(e, EntryNotFoundError)
                and not isinstance(e, LocalEntryNotFoundError)
                and (hasattr(e, "response") and e.response is not None and e.response.status_code == 404)
            ) or (hasattr(e, "response") and e.response is not None and e.response.status_code == 404)

...
def main():
    ...
    if args.mode == "merger":
        ...
        if success:
            # 【429防止】Discovery が取得済みのカタログ/マスタを Worker に配布するため保存
            for key, df in [("catalog", catalog.catalog_df), ("master", catalog.master_df)]:
                out_path = catalog.data_path / catalog.hf.paths[key]
                out_path.parent.mkdir(parents=True, exist_ok=True)
                df.to_parquet(out_path, index=False)
                logger.info(f"Artifact 配布用に保存: {out_path} ({len(df)} rows)")
    else:

        ...
        for row in meta:
            ...
            if not row.get("submitDateTime") and local_status == "failure":
                ...
                if not cat_rows.empty:
                    ...
                    if pd.notna(dt_obj):
                        row["submitDateTime"] = str(dt_obj)
                    row["filerName"] = str(cat_row.get("company_name") or "")
                    row["filerNameEn"] = str(cat_row.get("company_name_en") or "")
                    row["filerNameKana"] = str(cat_row.get("company_name_kana") or "")
                    row["edinetCode"] = str(cat_row.get("edinet_code") or "")
                    row["JCN"] = str(cat_row.get("jcn") or "")
                    row["secCode"] = str(cat_row.get("code") or "")
                    row["fundCode"] = str(cat_row.get("fund_code") or "")
                    row["docDescription"] = str(cat_row.get("title") or "")
                    row["docTypeCode"] = str(cat_row.get("doc_type") or "")
                    row["ordinanceCode"] = str(cat_row.get("ordinance_code") or "")
                    row["formCode"] = str(cat_row.get("form_code") or "")
                    row["parentDocID"] = str(cat_row.get("parent_doc_id") or "")
                    row["opeDateTime"] = str(cat_row.get("ope_date_time") or "")
                    row["legalStatus"] = str(cat_row.get("legal_status") or "0")
                    row["withdrawalStatus"] = str(cat_row.get("withdrawal_status") or "0")
                    row["docInfoEditStatus"] = str(cat_row.get("doc_info_edit_status") or "0")
                    row["disclosureStatus"] = str(cat_row.get("disclosure_status") or "0")
                    row["periodStart"] = str(cat_row.get("period_start") or "")
                    row["periodEnd"] = str(cat_row.get("period_end") or "")
                    ...
                    # 補完・属性情報
                    row["issuerEdinetCode"] = str(cat_row.get("issuer_edinet_code") or "")
                    row["subjectEdinetCode"] = str(cat_row.get("subject_edinet_code") or "")
                    row["subsidiaryEdinetCode"] = str(cat_row.get("subsidiary_edinet_code") or "")
                    row["currentReportReason"] = str(cat_row.get("current_report_reason") or "")
                    row["seqNumber"] = int(cat_row.get("seq_number") or 0)
                    row["AccountingStandardsDEI"] = str(cat_row.get("accounting_standard") or "")

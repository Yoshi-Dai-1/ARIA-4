...
from data_engine.core.utils import (
    aria_json_dump,
    aria_json_dumps,
    get_safe_int,
    get_safe_str,
...
def run_full_discovery(catalog, run_id):
    ...
    fetch_dates = set()
    h_start_date = None  # デフォルト: バックフィル期間なし（FINISHED 時のマトリックス分類に使用）
    ...
    if h_start:
        ...
        # バックフィル完了時: 過去60日間のリトライ窓を維持
        # 今日の新着書類 + 過去の failure 書類の自動救済を継続する
        limit_actual = LIMIT_DATE.date() if hasattr(LIMIT_DATE, "date") else LIMIT_DATE
        r_start = today - timedelta(days=60)
        if r_start < limit_actual:
            r_start = limit_actual
        curr = r_start
        while curr < today:
    ...
    for r_start, r_end in ranges:
        ...
        for row in meta:
            ...
            # 【Physical Truth First】EDINET APIが5年経過等でメタデータを隠蔽（Ghost化）している場合、
            # Our Catalog の不変メタデータを信用してリストアする
            if not row.get("submitDateTime") and local_status == "failure":
                cat_df = catalog.catalog_df
                cat_rows = cat_df[cat_df["doc_id"] == doc_id]
                if not cat_rows.empty:
                    import pandas as pd
                    cat_row = cat_rows.iloc[0]
                    dt_obj = cat_row.get("submit_at")
                    if pd.notna(dt_obj):
                        row["submitDateTime"] = get_safe_str(cat_row.get("submit_at"))
                    row["filerName"] = get_safe_str(cat_row.get("company_name"))
                    row["filerNameEn"] = get_safe_str(cat_row.get("company_name_en"))
                    row["filerNameKana"] = get_safe_str(cat_row.get("company_name_kana"))
                    row["edinetCode"] = get_safe_str(cat_row.get("edinet_code"))
                    row["JCN"] = get_safe_str(cat_row.get("jcn"))
                    row["secCode"] = get_safe_str(cat_row.get("code"))
                    row["fundCode"] = get_safe_str(cat_row.get("fund_code"))
                    row["docDescription"] = get_safe_str(cat_row.get("title"))
                    row["docTypeCode"] = get_safe_str(cat_row.get("doc_type"))
                    row["ordinanceCode"] = get_safe_str(cat_row.get("ordinance_code"))
                    row["formCode"] = get_safe_str(cat_row.get("form_code"))
                    row["parentDocID"] = get_safe_str(cat_row.get("parent_doc_id"))
                    row["opeDateTime"] = get_safe_str(cat_row.get("ope_date_time"))
                    row["legalStatus"] = get_safe_str(cat_row.get("legal_status"), "0")
                    row["withdrawalStatus"] = get_safe_str(cat_row.get("withdrawal_status"), "0")
                    row["docInfoEditStatus"] = get_safe_str(cat_row.get("doc_info_edit_status"), "0")
                    row["disclosureStatus"] = get_safe_str(cat_row.get("disclosure_status"), "0")
                    row["periodStart"] = get_safe_str(cat_row.get("period_start"))
                    row["periodEnd"] = get_safe_str(cat_row.get("period_end"))
                    row["xbrlFlag"] = "1" if cat_row.get("xbrl_flag") else "0"
                    row["pdfFlag"] = "1" if cat_row.get("pdf_flag") else "0"
                    row["englishDocFlag"] = "1" if cat_row.get("english_flag") else "0"
                    row["attachDocFlag"] = "1" if cat_row.get("attachment_flag") else "0"
                    row["csvFlag"] = "1" if cat_row.get("csv_flag") else "0"
                    
                    # 補完・属性情報
                    row["issuerEdinetCode"] = get_safe_str(cat_row.get("issuer_edinet_code"))
                    row["subjectEdinetCode"] = get_safe_str(cat_row.get("subject_edinet_code"))
                    row["subsidiaryEdinetCode"] = get_safe_str(cat_row.get("subsidiary_edinet_code"))
                    row["currentReportReason"] = get_safe_str(cat_row.get("current_report_reason"))
                    row["seqNumber"] = get_safe_int(cat_row.get("seq_number"), 0)
                    row["AccountingStandardsDEI"] = get_safe_str(cat_row.get("accounting_standard"))
    ...
    with open(meta_cache_path, "w", encoding="utf-8") as f:
        aria_json_dump(full_meta_cache, f, indent=2)
    ...
    # GHA matrix 出力
    print(f"JSON_MATRIX_PRIMARY: {aria_json_dumps(unique_p)}")
    print(f"JSON_MATRIX_RETRY: {aria_json_dumps(unique_r)}")

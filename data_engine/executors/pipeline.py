        ...
        for row in meta:
            ...
            if not row.get("submitDateTime") and local_status == "failure":
                ...
                if not cat_rows.empty:
                    ...
                    row["filerName"] = cat_row.get("company_name") or ""
                    row["filerNameEn"] = cat_row.get("company_name_en") or ""
                    row["filerNameKana"] = cat_row.get("company_name_kana") or ""
                    row["edinetCode"] = cat_row.get("edinet_code") or ""
                    row["JCN"] = cat_row.get("jcn") or ""
                    row["secCode"] = cat_row.get("code") or ""
                    row["fundCode"] = cat_row.get("fund_code") or ""
                    row["docDescription"] = cat_row.get("title") or ""
                    row["docTypeCode"] = cat_row.get("doc_type") or ""
                    row["ordinanceCode"] = cat_row.get("ordinance_code") or ""
                    row["formCode"] = cat_row.get("form_code") or ""
                    row["parentDocID"] = cat_row.get("parent_doc_id") or ""
                    row["opeDateTime"] = cat_row.get("ope_date_time") or ""
                    row["legalStatus"] = cat_row.get("legal_status") or "0"
                    row["withdrawalStatus"] = cat_row.get("withdrawal_status") or "0"
                    row["docInfoEditStatus"] = cat_row.get("doc_info_edit_status") or "0"
                    row["disclosureStatus"] = cat_row.get("disclosure_status") or "0"
                    row["periodStart"] = cat_row.get("period_start") or ""
                    row["periodEnd"] = cat_row.get("period_end") or ""
                    ...
                    # 補完・属性情報
                    row["issuerEdinetCode"] = cat_row.get("issuer_edinet_code") or ""
                    row["subjectEdinetCode"] = cat_row.get("subject_edinet_code") or ""
                    row["subsidiaryEdinetCode"] = cat_row.get("subsidiary_edinet_code") or ""
                    row["currentReportReason"] = cat_row.get("current_report_reason") or ""
                    row["seqNumber"] = cat_row.get("seq_number") or 0
                    row["AccountingStandardsDEI"] = cat_row.get("accounting_standard") or ""

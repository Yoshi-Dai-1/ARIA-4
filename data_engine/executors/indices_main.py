        ...
        if mode in ["all", "indices"]:
            ...
            for index_name in indices:
                ...
                try:
                    ...
                    # 前日のSnapshotを探す
                    try:
                        # 【Physical Truth First】HF上のディレクトリ走査により直近のSnapshotを物理的に取得 (予測排除)
                        try:
                            tree = catalog.hf.api.list_repo_tree(
                                repo_id=CONFIG.HF_REPO,
                                repo_type="dataset",
                                path_in_repo=f"master/indices/{index_name}/constituents/",
                                recursive=True
                            )
                            existing_parquets = [item.path for item in tree if item.path.endswith('.parquet')]
                            
                            target_val = target_date.replace("-", "")
                            best_path = None
                            best_val = ""
                            
                            for p in existing_parquets:
                                # パスから YYYYMMDD を抽出 (例: data_20260327.parquet)
                                # master/indices/../constituents/year=.../data_YYYYMMDD.parquet
                                p_filename = p.split("/")[-1]
                                p_val = p_filename.replace("data_", "").replace(".parquet", "")
                                
                                # target_val未満で最大のものを探す
                                if p_val < target_val and p_val > best_val:
                                    best_val = p_val
                                    best_path = p
                            ...
                            if best_path:
                                prev_date = f"{best_val[:4]}-{best_val[4:6]}-{best_val[6:]}"
                                logger.info(f"Found Previous Snapshot in Repo: {best_path} ({prev_date})")
                                
                                from huggingface_hub import hf_hub_download
                                
                                # ダウンロード失敗時はフェイルファスト (安易な空DataFrameへのフォールバックを排除)
                                hf_hub_download(
                                    repo_id=CONFIG.HF_REPO,
                                    filename=best_path,
                                    repo_type="dataset",
                                ...
                                downloaded_path = temp_dir / best_path
                                if downloaded_path.exists():
                                    ...
                                    logger.error(f"Snapshot local verification failed: {downloaded_path}")
                                    raise FileNotFoundError("Downloaded Snapshot could not be found locally.")
                            else:
                                logger.info(f"No previous snapshots found before {target_date}. Treat as initial run.")
                        
                        except Exception as e_dl:
                            # 【フェイルファスト】API通信エラーなど、致命的エラー時は処理を完全停止する
                            logger.critical(
                                f"Failed to resolve Previous Snapshot for {index_name}. "
                                f"Aborting to prevent data corruption. Error: {e_dl}"
                            )
                            raise e_dl
                        ...
                        if not diff_events.empty:
                            ...
                            try:
                                ...
                                if is_404(e):
                                    logger.info(f"Initial history for {index_name} (File not found on HF).")
                                    df_hist_current = pd.DataFrame(columns=history_cols)
                                else:
                                    # 通信障害(503)等の場合は、空データで上書きすることを防ぐためFail-Fastする
                                    logger.critical(f"Aborting Index Update due to network error: {e}")
                                    raise

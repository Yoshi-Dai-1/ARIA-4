...
from data_engine.core.utils import get_tar_repo_path
...
from data_engine.storage.tar_bundle_manager import extract_file_from_tar
...
def run_backfill(catalog: CatalogManager, run_id: str, limit: int = None):
    ...
    condition_status = ~df_catalog["processed_status"].isin(["success", "retracted", "unsupported_taxonomy"])
    ...
    for _, row in df_targets.iterrows():
        ...
        try:
            ...
            if is_404(e):
                ...
                tar_repo = get_tar_repo_path(submit_at)
                if tar_repo:
                    try:
                        ...
                        # TAR 内のメンバー名を導出（raw_zip_path から逆算）
                        member_name = f"zip/{doc_id}.zip"
                        dest = Path(CONFIG.DATA_PATH) / raw_zip_path
                        dest.parent.mkdir(parents=True, exist_ok=True)
                        if extract_file_from_tar(Path(tar_local), member_name, dest):
                            local_zip = str(dest)
                            logger.info(f"ZIP Recovered from TAR for backfill: {tar_repo} -> {doc_id}")
                        else:
                            logger.error(f"ZIP not found in TAR for {doc_id}.")
                            record["processed_status"] = "failure"
                            continue
                    except Exception as tar_e:
                        if is_404(tar_e):
                            logger.error(f"ZIP not found (individual & TAR) for {doc_id}.")
                        else:
                            logger.critical(f"Aborting Backfill XBRL due to network error during TAR ZIP download: {tar_e}")
                            raise
                        record["processed_status"] = "failure"
                        continue
                else:
                    logger.error(f"ZIP not found on HF for {doc_id} (404).")
                    record["processed_status"] = "failure"
                    continue
            else:
                # 503 等の通信障害時は、カタログを failure に書き換えずプロセスを停止
                logger.critical(f"Aborting Backfill XBRL due to network error during ZIP download: {e}")
                raise

...
def _call_with_retry(func, max_retries: int = 5):
    """HF API 呼び出しをリトライ付きで実行する。

    Harvester (hf_storage._upload_with_retry) と同等のリトライ戦略:
    - 429 (Rate Limit): Retry-After ヘッダー + マージン 5 秒を待機して再試行
    - 500+ (Server Error): 段階的バックオフ（15秒 × 試行回数）で再試行
    - その他の一時エラー: 段階的バックオフ（10秒 × 試行回数）で再試行
    - 404 や引数エラーなどの回復不能なエラーはリトライせず即座に raise
    """
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # 429 Rate Limit
            if isinstance(e, HfHubHTTPError) and e.response is not None and e.response.status_code == 429:
                wait_time = int(e.response.headers.get("Retry-After", 300)) + 5
                if not is_last_attempt:
                    logger.warning(
                        f"  ⏳ HF API レート制限 (429)。{wait_time} 秒待機後にリトライします "
            ...
            # 500+ Server Error
            if isinstance(e, HfHubHTTPError) and e.response is not None and e.response.status_code >= 500:
                wait_time = 15 * attempt
                if not is_last_attempt:
                    logger.warning(
                        f"  ⏳ HF サーバーエラー ({e.response.status_code})。{wait_time} 秒待機後にリトライします "
                        f"(試行 {attempt}/{max_retries})"
                    )
                    time.sleep(wait_time)
                    continue
            ...
            # 404 や引数エラーなど、リトライしても回復しないエラーは即座に raise
            if isinstance(e, HfHubHTTPError) and e.response is not None and e.response.status_code < 500:
                raise

            # ネットワークタイムアウト等の一時的なエラー
            wait_time = 10 * attempt
            if not is_last_attempt:
                logger.warning(
                    f"  ⏳ 一時的なエラー。{wait_time} 秒待機後にリトライします "
                    f"(試行 {attempt}/{max_retries}): {e}"
                )
                time.sleep(wait_time)
                continue
            raise
...
def migrate_month(
    ...
    try:
        raw_items = _call_with_retry(lambda: list(api.list_repo_tree(
    ...
    try:
        tar_items = _call_with_retry(lambda: list(api.list_repo_tree(
            repo_id=hf_repo, repo_type="dataset",
    ...
    for day in sorted(day_groups.keys()):
        ...
        try:
            ...
            for idx, repo_path in enumerate(files, 1):
                try:
                    local = _call_with_retry(lambda rp=repo_path: hf_hub_download(
                        repo_id=hf_repo,
                        filename=rp,
                        repo_type="dataset",
                        token=hf_token,
            ...
            if merge_existing:
                ...
                try:
                    existing_local = _call_with_retry(lambda: hf_hub_download(
                        repo_id=hf_repo,
            ...
            _call_with_retry(lambda: api.create_commit(
                repo_id=hf_repo,
                repo_type="dataset",
            ...
            # create_commit はアトミック。例外なく返った時点でコミットは成功している。
            # 追加の API 呼び出し（repo_info 等）はレート制限 (429) を誘発するため行わない。

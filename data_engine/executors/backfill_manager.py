...
def load_cursor():
    ...
    try:
        ...
        if is_404(e):
            logger.warning(f"Cursor not found on HF (First run?): {e}")
            return None
        else:
            # 503 等の通信障害時は、カーソルを初期化（10年巻き戻し）せず Fail-Fast する
            logger.critical(f"Aborting Backfill Manager due to network error during cursor load: {e}")
            raise
...
def save_cursor(next_start_date_str):
    ...
    from data_engine.core.utils import aria_json_dump
    META_DIR.mkdir(parents=True, exist_ok=True)
    with open(local_path, "w") as f:
        aria_json_dump(cursor_data, f, indent=2)

...
class DataReconciliationEngine:
    ...
    def reconcile_physical_assets(self, sample_size: int = 50):
        ...
        try:
            ...
            # TAR バンドルファイルも収集（移行後の物理形態）
            tar_files = set([f for f in raw_files if f.endswith(".tar")])
            ...
            # 存在確認 (Existence check): 個別ファイル OR 対応する TAR ファイルのいずれかが存在すればOK
            missing_zips = []
            for doc_id, path in expected_zips.items():
                if path in raw_files:
                    continue  # 個別ファイルが存在
                # TAR バンドルの存在確認
                submit_at = catalog_df.loc[catalog_df["doc_id"] == doc_id, "submit_at"].iloc[0]
                tar_path = utils.get_tar_repo_path(str(submit_at)) if submit_at else None
                if tar_path and tar_path in tar_files:
                    continue  # TAR バンドルが存在
                missing_zips.append(doc_id)

            missing_pdfs = []
            for doc_id, path in expected_pdfs.items():
                if path in raw_files:
                    continue
                submit_at = catalog_df.loc[catalog_df["doc_id"] == doc_id, "submit_at"].iloc[0]
                tar_path = utils.get_tar_repo_path(str(submit_at)) if submit_at else None
            ...
            for bf in bin_files:
                ...
                try:
                    ...
                    try:
                        ...
                        if is_404(e):
                            logger.error(f"Bin file {bf} is physically missing (404).")
                        else:
                            # 503等の発生時は監査を中断し、カタログの誤った書き換えを防ぐ
                            logger.critical(f"Aborting Audit due to network error during bin download: {e}")
                            raise
                        ...
                        if self.repair:
                            ...
                            for commit_hash in history[1:]:
                                try:
                                    ...
                                    if not is_404(e_rollback):
                                        # ロールバック試行中の通信障害も致命的とみなす
                                        continue
                    ...
                    if doc_col in df.columns and not df.empty:
                        actual_docs = set(df[doc_col].dropna().unique())
                    else:
                        actual_docs = set()

                    # 1. 重複チェック (ID + 値の完全一致のみ排除)
                    if "key" in df.columns and not df.empty:
                        dup_keys = [doc_col, "key", "context_ref"]
                        if "data_str" in df.columns:
                            dup_keys.append("data_str")
                    ...
                    if unrecognized_docs:
                        ...
                        if self.repair:
                            ...
                            df = df[~df[doc_col].isin(unrecognized_docs)]

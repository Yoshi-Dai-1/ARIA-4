...
def get_tar_repo_path(submit_at: str) -> Optional[str]:
    """
    日次 TAR バンドルの HF リポジトリ内パスを導出する（SSOT）。
    例: submit_at="2024-10-23 15:00" → "raw/edinet/2024/10/23.tar"
    """
    if not submit_at or len(str(submit_at)) < 10:
        return None
    ...
    try:
        y, m, day = d.split("-")
        return f"raw/edinet/{y}/{m}/{day}.tar"
    except Exception:
        return None
...
def log_resources(label: str):
    ...
    try:
        ...
        # 工学的主権（Fact-based Robustness）に基づく設計:
        # GitHub Actions 等のWeb環境では、フォントやレンダリングエンジンによって全角・半角の幅の比率が異なるため、
        # スペースによるパディングで「右側」の数値を揃えるアプローチは視覚的に破綻する。
        # これを完全に解決するため、必ず「固定幅のメトリクス」を左側に、可変長のラベルを右側に配置する。
        logger.info(
            f"📊 [Mem: {rss_mb:4.0f} MB | Disk: {disk_free_gb:5.1f} GB ({disk_used_pct:4.1f}%)] {label}"
        )

...
def is_404(e: Exception) -> bool:
    ...
    False を返却することで、誤ったデータ初期化（サイレント消失）を防止する。

    【多層防御】hf_hub_download は HEAD リクエストの 429 失敗を
    EntryNotFoundError としてラップして raise する場合がある。
    この誤分類を検知するため、以下の2層でガードする:
      1) .response のステータスコードが 404 以外であれば偽陽性と判定
      2) 例外チェーン(__cause__/__context__)に 429/rate limit の痕跡があれば偽陽性と判定
    """
    # 1. Hugging Face Hub 特有の "File Not Found" 例外
    if isinstance(e, EntryNotFoundError):
        # 【ガード層1】.response が存在する場合、ステータスコードで真偽を確認
        resp = getattr(e, "response", None)
        if resp is not None:
            sc = getattr(resp, "status_code", None)
            if sc is not None and sc != 404:
                return False  # 429/503等 → 物理的に 404 ではない
        # 【ガード層2】例外チェーンに 429/rate limit の痕跡があれば偽陽性
        chain = getattr(e, "__cause__", None) or getattr(e, "__context__", None)
        if chain:
            chain_str = str(chain).lower()
            if "429" in chain_str or "rate limit" in chain_str or "too many request" in chain_str:
                return False
        return True
...
def patch_all_networking():
    ...
    # 1. HuggingFace Hub の通信堅牢化
    # 【設計判断 2026-09-04】get_session のモンキーパッチを撤廃。
    # 理由: huggingface_hub 1.x は httpx ベースの堅牢なリトライ機構を内蔵しており、
    # requests.Session をラップした HttpxCompatibilityProxy は httpx.Response 固有の
    # 属性（has_redirect_location 等）の欠落を引き起こし、CI 障害の原因となった。
    # HF タイムアウト設定は hf_storage.py の __init__ で実施済みのため、ここでは不要。
    # hf_storage.py の load_parquet 内の10回リトライ + 指数バックオフが多層防御として機能する。
    logger.info("HF Hub communication relies on native httpx retry mechanism.")

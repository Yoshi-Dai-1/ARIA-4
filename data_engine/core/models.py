...
def pydantic_to_pyarrow(model_class) -> pa.Schema:
    ...
    for name, info in model_class.model_fields.items():
        ...
        pa_type = _PYTHON_TO_PYARROW.get(py_type, pa.large_string())
...
# ストリーミングマージ用バッチサイズ (master_merger._merge_with_duckdb が参照)
# qualitative_text は 1行あたり数十〜数百KB の data_str を含むため、
# 小さいバッチサイズを使用してピークメモリを抑制する。
MERGE_BATCH_SIZES: dict[str, int] = {
    "financial_values": 50_000,  # 1行 ~1KB → 50MB/バッチ
    "qualitative_text": 2_000,   # 1行 ~100KB → 200MB/バッチ
}
MERGE_BATCH_SIZE_DEFAULT: int = 10_000
